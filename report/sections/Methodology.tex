...
In \hyperlink{subsection.5.2}{Section 5.2}, the experimentation setup pertaining to the type of optimizer used, number of training iterations, and learning rate is given. Although the above is technical, GPs have an intuitive procedure for training and performing inference. First, prior beliefs are encoded using kernel functions.  Second, condition on what data has been observed. Third, optimize the parameters. Lastly, use the posterior predictive distribution for performing inference.
